                ...
                for user in users:
                    ...
                    for codefile in user.codefiles.values():
                        ...
                        if codefile.exercise_have_start_code:
                            ...
                            # Não devo usar usar esse condicional porque isso limita muito quantos alunos entrarão
                            # no relatóiro. O próprio cálculo da assertividade já a define como 0.0 nos seguintes casos:
                            # (1) se o aluno não obteve grade 100% (não passou em todos os CTs)
                            # (2) se houve erro no codemirror, isto é, erro no registro do número de digitações de
                            #     adição de caracteres (ADD) e/ou na deleção de caracteres (DEL). Se
                            # (3) há outras siutações (veja arquivo Assertividade-certificador.key)
    ...
    def _process_report_assertiveness_by_student_assessments(self, df_assertiv_exercise: pd.DataFrame):
        ...
        for index, row in df_assertiv_exercise.iterrows():
            ...
            if user_pred == row['USER_ID'] and assessment_pred == row['ASSESSMENT_TYPE']:
                ...
                # chamados de grade.
                num_score = round(assertiv_value, self.DECIMAL_PLACES)
                assertiv_value_list.append(num_score * 10) # *10 para num_score ficar entre 0.0 e 10.0

                # Obtém um conceito IRBO a partir do valor da assertividade, isto é, num_score.
                assertiv_IRBO_grade_list.append(Helper.IRBO_scale("A", num_score*10))  # *10 para num_score ficar entre 0.0 e 10.0
        ...
        user_id_list.append(user_pred)
        num_score = round(assertiv_value, self.DECIMAL_PLACES)
        assertiv_value_list.append(num_score)
        assertiv_IRBO_grade_list.append(Helper.IRBO_scale("A", num_score*10))
        ...
        df = pd.DataFrame({'ID': id_list,
                           ...
                           'USER_ID': user_id_list,
                           'ASSERTIV_VALUE': assertiv_value_list,
                           'IRBO_GRADE': assertiv_IRBO_grade_list,
                           })
